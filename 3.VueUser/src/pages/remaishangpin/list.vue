...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 2%","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>手机名称：</div>
        <el-input v-model="formSearch.shangpinmingcheng" placeholder="手机名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	  ...
	  <el-button v-if="btnAuth('remaishangpin','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 25px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#57A7A5","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/remaishangpinAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"margin":"10px 0","order":"3"}'>
		<el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px","background":"none"}' @click="sortClick('price')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#000"}' class="icon iconfont icon-chujia13" v-if="sortType!='price'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#000"}' class="icon iconfont icon-jiantou35" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#000"}' class="icon iconfont icon-jiantou36" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 2% 10px 0","overflow":"hidden","background":"#fff","width":"31.3%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			...
			<div class="content">
		      <div class="info" :style='{"width":"50%","padding":"0 0 0 10px","flexWrap":"wrap","flex":"1","display":"flex","height":"100%"}'>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">{{item.shangpinmingcheng}}</div>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">商家名称:{{item.shangjiamingcheng}}</div>
		        ...
		        <div :style='{"padding":"0","display":"none"}'>
		          ...
		          <span :style='{"color":"#fff","lineHeight":"1.5","fontSize":"12px"}'>{{item.shangjiamingcheng}}</span>
		        </div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/pinpai/pinpai').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getFenlei() {
		...
		this.feileiColumn = 'shangpinleibie'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.shangpinleibie = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`remaishangpin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/remaishangpinDetail', query: params});
      },
