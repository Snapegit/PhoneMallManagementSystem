...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 2%","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>手机类别：</div>
        <el-input v-model="formSearch.shangpinleibie" placeholder="手机类别" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('shangpinleibie','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 25px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#57A7A5","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shangpinleibieAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"0 0 10px","background":"none","order":"4"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 2% 10px 0","overflow":"hidden","background":"#fff","width":"31.3%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			<img @click.stop="imgPreView(item.image)" v-if="item.image && item.image.substr(0,4)=='http'" :src="item.image.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.image?item.image.split(',')[0]:''))" v-else :src="baseUrl + (item.image?item.image.split(',')[0]:'')" class="image" />
			<div class="content">
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.shangpinleibie != '') searchWhere.shangpinleibie = '%' + this.formSearch.shangpinleibie + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`shangpinleibie/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/shangpinleibieDetail', query: params});
      },
